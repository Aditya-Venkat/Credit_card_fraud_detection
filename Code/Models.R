...
cat("Isolation Forest Accuracy:", mean(val_data$iso_anomaly == val_data$is_fraud), "\n")
...
db_val <- dbscan(scale(val_data[, c("amt", "age", "city_pop", "hour", "distance")]), eps = 1.5, minPts = 10)
val_data$dbscan_anomaly <- ifelse(db_val$cluster == 0, 1, 0)
cat("DBSCAN Accuracy:", mean(val_data$dbscan_anomaly == val_data$is_fraud), "\n")
